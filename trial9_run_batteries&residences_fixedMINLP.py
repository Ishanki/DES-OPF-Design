...
from trial9_residences_fixedMINLP import Residential_Seasonal_DES
from trial9_DES_OPF import DES_OPF

'''
This is the run file for the model that, for a given case study,
1) runs an MILP considering all 4 seasons 
2) fixes all the capacities and binary decisions
3) runs an NLP based on the fixed decisions with OPF constraints   
'''
m = ConcreteModel()
...
irrad_file_name = "Parameters_cmbnd.xlsx" #giving the flexibility to change to a seasonal file
...
results_file_name = 'NLP_season_' #looks incomplete but it will attach the string of season later
results_file_suffix = '.xlsx'
df_grid = pd.read_excel(parameters_file_name, sheet_name = "Grid")
S_base = df_grid.iat[5,1]  #used in the linking vars
...
'''fixing MIP solution for NLP run'''
for s in m.S:
    for h in m.DES_res[s].i:
        ...
        for t in m.DES_res[s].t:
            ...
            for c in m.DES_res[s].c:
                ...
                m.DES_res[s].Q1[h,t,c].fix()
                m.DES_res[s].Q2[h,t,c].fix()
...
df_scalars = pd.read_excel(parameters_file_name, sheet_name = "Res_Scalars")
PF = df_scalars.iat[17,1] # doc = 'PV inverter power factor')

'''reactive power generation'''
def Inv_q_gen(m,s,i,t):
    return m.DES_res[s].Q_gen[i,t] == sqrt((((m.DES_res[s].P_inv[i,t])**2)*(1-(PF**2))/(PF**2)) + 0.0001)
#sqrt((model.S_inv[i,t]**2) - (model.P_inv[i,t]**2) + 0.0001)
m.INV6 = Constraint(m.S, house, m.timestamp, rule = Inv_q_gen)
