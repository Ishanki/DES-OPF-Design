...
from MILP import ResidentialSeasonalDES
...
#                              '''Inputs here'''
# =============================================================================
loads_file_name = "Profiles_BM.xlsx" #"summer.xls" #"Loads_houses.xls"
...
KEEP_OPF = 1
I_MAX = 250 #Leave None or else leave a value in Amps for max line current
...
with open("MILP_results.txt","w") as f:
    f.write(str(results))
...
if KEEP_OPF == 1:
    ...
    if RUN_NLP == 1:
        ...
        with open("CONOPT_results.txt","w") as f:
            f.write(str(results))
...
for s in m.S:
    ...
    Q_res_data = {(i, t): value(v) for (i,t), v in m.DES_res[s].Q_load.items()}
