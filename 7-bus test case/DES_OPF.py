...
class DES_OPF(object):
    ...
    def OPF(self,power_init=None,load_init=None):
        ...
        # for key,v in non_diags.items():
            y_nd[key[0],key[1]] = - sum(v for (k1,k2),v in Y.items() if key[0]==k1 and key[1]==k2) - \
                sum(v for (k1,k2),v in Y.items() if key[0]==k2 and key[1]==k1)
        #print("NON-DIAGONALS")
        # print(y_nd)
        ...
        model.P = Var(model.n, model.t,  bounds = (None,None), doc = 'node active power', initialize = P_init)
        model.Q = Var(model.n, model.t,  bounds = (None,None), doc = 'node reactive power', initialize = Q_init)
