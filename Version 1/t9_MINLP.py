...
class ResidentialSeasonalDES(object):
    ...
    def DES_MINLP(self):
        ...
        def PV_investment(model):
            return (sum(CC_PV * model.panels_PV[i] * model.CRF for i in model.i))/4 == model.annual_inv_PV
        ...
        def PV_operation_cost(model):
            return sum(((model.E_PV_used[i,t] + model.E_PV_sold[i,t] + sum(model.E_PV_charge[i,t,c] for c in model.c))* OC_VAR_PV * self.days * self.interval) for i in model.i for t in model.t) \
        ...
        def Inv_q_gen(model,i,t):
            return model.Q_gen[i,t] == 0
        # sqrt((((model.P_inv[i,t])**2)*(1-(PF**2))/(PF**2))+0.0001)
        # model.Q_gen[i,t]**2 == ((model.P_inv[i,t])**2)*(1-(PF**2))/(PF**2)
        # model.Q_gen[i,t] == sqrt((((model.P_inv[i,t])**2)*(1-(PF**2))/(PF**2))+0.0001)
        ...
        def boiler_investment(model):
            return (sum(CC_B * model.max_H_b[i] * model.CRF for i in model.i))/4 == model.annual_inv_B
        ...
        def charging_bigM(model,i,t,c):
            return model.E_charge[i,t,c] <= 100 * model.Q[i,t,c]
        ...
        def discharging_bigM(model,i,t,c):
            return model.E_discharge[i,t,c] <= 100 * (1-model.Q[i,t,c])
        ...
        #sum(((model.E_grid[i,t] + model.E_grid_charge[i,t,c]) * self.interval * PRICE_GRID * self.days) for i in model.i for t in model.t for c in model.c)
